    ...
    $.extend(Fastsearch.prototype, {
        ...
        events: function () {
            ...
            options.mouseEvents &&
                this.$el
                    .on(
                        this.namespaceEvents("mouseleave"),
                        this.itemSelector,
                        function (e) {
                    ...
                    .on(
                        this.namespaceEvents("mouseenter"),
                        this.itemSelector,
                        function (e) {
                            self.$resultItems.removeClass(
                                options.focusedItemClass
                            )
            ...
            if (inputValue.length < this.options.minQueryLength) {
                ...
                this.keyupTimeout = setTimeout(function () {
                    ...
                    self.getResults(function (data) {
                        self.showResults(
                            self.storeResponse(data).generateResults(data)
                        )
                    })
        ...
        generateItem: function (item) {
            ...
            options.onItemCreate &&
                options.onItemCreate.call(this, $tag, item, this)
        ...
        checkDropdownPosition: function () {
            ...
            var offset =
                typeof flipOnBottom === "boolean" && flipOnBottom
                    ? 400
                    : flipOnBottom
            var isFlipped =
                this.$input.offset().top + offset > $document.height()
        ...
        navigateItem: function (direction) {
            var $currentItem = this.$resultItems.filter(
                    this.focusedItemSelector
                ),
...
;(function (root, factory) {
    ...
    $.extend(Fastselect.prototype, {
        ...
        setupDomElements: function () {
            ...
            this[
                this.isMultiple ? "setupMultipleElement" : "setupSingleElement"
            ](function () {
        ...
        processInitialOptions: function () {
            ...
            if (this.hasCustomLoader) {
                ...
                options = $.map(
                    this.$input.find("option:selected").get(),
                    function (option) {
                        var $option = $(option)
        ...
        setupFastsearch: function () {
                ...
                $.extend(fastSearchParams, {
                    ...
                    onItemSelect: function ($item, model, fastsearch) {
                        ...
                        if (
                            self.isMultiple &&
                            maxItems &&
                            self.optionsCollection.getValues().length >
                                maxItems - 1
                        ) {
                            options.onMaxItemsReached &&
                                options.onMaxItemsReached(this)
                        } else {
                    ...
                    onItemCreate: function ($item, model) {
                        model.$item = $item
                        model.selected &&
                            $item.addClass(options.itemSelectedClass)

                        if (self.userOptionAllowed && model.isUserOption) {
                            $item
                                .text(
                                    self.options.userOptionPrefix + $item.text()
                                )
            ...
            this.fastsearch.getResults = function () {
                if (
                    self.userOptionAllowed &&
                    self.$queryInput.val().length > 1
                ) {
                    self.renderOptions()
                }
            ...
            if (this.hasCustomLoader) {
                ...
                !this.optionsCollection.models &&
                    this.optionsCollection.reset(
                        this.gleanSelectData(this.$input)
                    )
                onDone()
            }
        },
        ...
        adjustQueryInputLayout: function () {
            if (this.isMultiple && this.$queryInput) {
                var noneSelected = this.$el.hasClass(
                    this.options.noneSelectedClass
                )
                ...
                if (noneSelected) {
                    ...
                    this.$fakeInput.text(
                        this.$queryInput.val().replace(/\s/g, "&nbsp;")
                    )
                    this.$queryInput
        ...
        documentCancelEvents: function (setup) {
            Fastsearch.prototype.documentCancelEvents.call(
                this,
                setup,
                this.hide
            )
        ...
        setSelectedOption: function (option) {
            ...
            var selectedModel = this.optionsCollection.findWhere(function (
                model
            ) {
    ...
    $.extend(OptionsCollection.prototype, {
        ...
        fetch: function (fetchParams, onDone) {
            ...
            if (this.options.loadOnce) {
                this.fetchDeferred =
                    this.fetchDeferred || this.load(fetchParams)
